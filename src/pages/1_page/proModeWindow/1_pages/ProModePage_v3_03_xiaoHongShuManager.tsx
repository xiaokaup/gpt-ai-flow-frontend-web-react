...
export const ProModePage_v3_03_xiaoHongShuManager = (props: IProModePage_xiaoHongShu_input) => {
  ...
  const handleContextTypeChange = (paraContextType: EProMode_v3_03_xiaoHongShuManager_contextType) => {
    ...
    if (!contextPrompts[paraContextType].stages[contextTypeStage].value) {
      handleContextTypeStageChange(EProMode_v3_03_xiaoHongShuManager_contextTypeStage.DEFAULT);
    }
  ...
  const handleContextTypeStageChange = (paraContextTypeStage: EProMode_v3_03_xiaoHongShuManager_contextTypeStage) => {
            ...
            <Select
              ...
              options={defaultContextTypeStagesForSelect
                .map((item) => {
                  return {
                    label:
                      contextPrompts[contextType].stages[item as EProMode_v3_03_xiaoHongShuManager_contextTypeStage]
                        .name,
                    value: item,
                    disabled:
                      contextPrompts[contextType].stages[item as EProMode_v3_03_xiaoHongShuManager_contextTypeStage]
          ...
          <div className="column" style={{ display: 'flex', alignItems: 'center' }}>
            ...
            <Select
              ...
              options={defaultContextTypesForSelect
                .map((item) => {
                  return {
                    label: contextPrompts[item as EProMode_v3_03_xiaoHongShuManager_contextType].name,
                    value: item,
                    disabled: contextPrompts[item as EProMode_v3_03_xiaoHongShuManager_contextType].disable,
                  };
                })
                .filter((item) => !item.disabled || item.label)}
            />
