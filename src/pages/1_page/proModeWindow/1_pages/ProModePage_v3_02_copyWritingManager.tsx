...
export const ProModePage_v3_02_copyWritingManager = (props: IProModePage_copyWriting_input) => {
  ...
  const handleContextTypeChange = (paraContextType: EProMode_v3_02_copyWritingManager_contextType) => {
    ...
    if (!contextPrompts[paraContextType].stages[contextTypeStage].value) {
      handleContextTypeStageChange(EProMode_v3_02_copyWritingManager_contextTypeStage.DEFAULT);
    }
            ...
            <Select
              ...
              options={defaultContextTypeStagesForSelect
                .map((item) => {
                  return {
                    label:
                      contextPrompts[contextType].stages[item as EProMode_v3_02_copyWritingManager_contextTypeStage]
                        .name,
                    value: item,
                    disabled:
                      contextPrompts[contextType].stages[item as EProMode_v3_02_copyWritingManager_contextTypeStage]
              ...
              options={defaultContextTypesForSelect
                .map((item) => {
                  return {
                    ...
                    value: item,
                    disabled: contextPrompts[item as EProMode_v3_02_copyWritingManager_contextType].disable,
                  };
                })
                .filter((item) => !item.disabled || item.label)}
            />
