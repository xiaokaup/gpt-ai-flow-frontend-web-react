...
import { webAppUrl, isHomePage, docBaseUrl } from './constant';
import { IOneFeature } from './interface';

const { Search } = Input;

const getModules = (isHomePage: boolean, locale: string) => {
...
export const ProModeWindowFeatures_module = (props: IProModeWindowFeatures_module_input) => {
  const {
    locale,
    // location
  } = props;

  const features = getModules(isHomePage, locale);
